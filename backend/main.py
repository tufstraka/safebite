...
class NovaMenuAnalyzer:
    ...
    async def analyze_menu_image(self, image_data: bytes) -> Dict:
    ...
    async def assess_dish_safety(self, dish: Dict, allergens: List[str]) -> DishSafety:
...
async def analyze_menu_image(file: UploadFile = File(...), allergens: str = ""):
    """
    Analyze menu from uploaded image
    ...
    try:
        ...
        menu_data = await analyzer.analyze_menu_image(image_data)
        ...
        for dish in menu_data["dishes"]:
            safety = await analyzer.assess_dish_safety(dish, allergen_list)
        ...
        voice_summary = await analyzer.generate_voice_summary(
            len(safe_dishes), len(unsafe_dishes), allergen_list
        )
...
async def analyze_menu_url(request: MenuAnalysisRequest):
    ...
    try:
        ...
        for dish in menu_data["dishes"]:
            safety = await analyzer.assess_dish_safety(dish, request.allergens)
        ...
        # Generate voice summary
        voice_summary = await analyzer.generate_voice_summary(
            len(safe_dishes), len(unsafe_dishes), request.allergens
        )
